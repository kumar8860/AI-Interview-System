...
const Interview = () => {
  ...
  const startListening = (sid) => {
    if (status === "processing" || isFinished) return;
      ...
      async (ans) => {
        ...
        try {
          const res = await fetch("http://localhost:5000/interview", {
            ...
            body: JSON.stringify({ session_id: sid, answer: ans, code: userCode })
          });
          const data = await res.json();
          engineControl(data.next_question, sid, data.is_final);
        } catch (e) {
    ...
    <div style={ui.page}>
      <div style={ui.wrapper}>
        ...
        {!isFinished ? (
          <div style={ui.grid}>
            <aside style={ui.aside}>
              <div style={ui.camContainer}>
                <Camera onMetricsUpdate={handleMetrics} />
              </div>
          ...
          <ScoreBoard finalFeedback={question} />
        )}
      </div>
    </div>
  );
...
const ui = {
  ...
  status: (s) => ({ padding: "4px 14px", borderRadius: "4px", fontSize: "0.6rem", fontWeight: "bold", background: s === "listening" ? "#ef4444" : "#1e293b", color: "#fff" }),
  ...
  actionBtn: { padding: "18px", borderRadius: "12px", background: "linear-gradient(135deg, #3b82f6, #1e40af)", color: "#fff", fontWeight: "bold", border: "none", cursor: "pointer" },
  ...
  abortBtn: { alignSelf: "flex-start", marginTop: "30px", background: "none", border: "1px solid #374151", color: "#6b7280", padding: "5px 12px", borderRadius: "4px", fontSize: "0.7rem", cursor: "pointer" }
};
